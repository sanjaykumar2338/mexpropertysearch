<div class="item">
	...
	if ( $settings['item_masonry'] == 'masonry8' ) {
		$arr_index_large_image = array( 0, 1, 6, 7 );
		$width_image           = ( $settings['style_masonry'] == 'yes' && in_array( $index, $arr_index_large_image ) ? '825' : '550' );
		$height_image          = ( $settings['style_masonry'] == 'yes' && in_array( $index, $arr_index_large_image ) ? '914' : '354' );
	} else {
		$arr_index_large_image = array( 0, 5 );
		$width_image           = ( $settings['style_masonry'] == 'yes' && in_array( $index, $arr_index_large_image ) ? '825' : '550' );
		$height_image          = ( $settings['style_masonry'] == 'yes' && in_array( $index, $arr_index_large_image ) ? '769' : '354' );
	}
	?>
	<div class="area-post area-post-<?php echo esc_attr( $taxonomy->term_id ); ?> <?php echo esc_attr( $settings['show_link_listing'] == 'yes' ? 'has-button' : '' ); ?>">
		<div class="featured-post">
			<a href="<?php echo esc_url( $term_link ) ?>" class="image-wrap">
				<span class="icon-plus"><img loading="lazy" src="<?php echo esc_attr( TF_PLUGIN_URL ); ?>includes/elementor-widget/assets/images/icons/Icon-hover.svg" alt="images"></span>
				<?php
