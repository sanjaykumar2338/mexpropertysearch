 ...
 * @var $column_properties_list
 * @var $item_style_properties_grid
 * @var $item_style_properties_list
 */
if (!defined('ABSPATH')) {
...
$map_position = $map_position ? $map_position : tfre_get_option('map_position');
$archive_property_search_form = $search_form ? $search_form : tfre_get_option('archive_property_search_form');
$archive_property_sidebar = $sidebar ? $sidebar : tfre_get_option('archive_property_sidebar');
$archive_property_sidebar_position = $sidebar_position ? $sidebar_position : tfre_get_option('archive_property_sidebar_position');
$column_layout_grid = $column_properties_grid ? $column_properties_grid : tfre_get_option('column_layout_grid');
$item_style_layout_grid = $item_style_properties_grid ? $item_style_properties_grid : tfre_get_option('item_style_layout_grid');
$column_layout_list = $column_properties_list ? $column_properties_list : tfre_get_option('column_layout_list');
$item_style_layout_list = $item_style_properties_list ? $item_style_properties_list : tfre_get_option('item_style_layout_list');
...
if ($layout_archive_property == 'grid') {
	...
	$style_layout_column = $column_layout_list;
}
...
<?php if ($map_position != 'hide-map' && $map_position == 'map-header'): ?>
	<div class="map-container <?php echo esc_attr(tfre_get_option('hide_map_mobile') == 'y' ? 'hide-map-mobile' : ''); ?> <?php echo esc_attr( $map_position ); ?>"
		style="height:600px">
		<div id="map" class="no-fixed"></div>
	</div>
<?php endif; ?>

<?php $archive_property_sidebar !== 'enable' && $archive_property_search_form == 'enable' && do_action('property_advanced_search_form'); ?>

<div class="tfre_message"></div>

<div
	class="cards-container <?php echo esc_attr($map_position); ?> <?php echo esc_attr($archive_property_sidebar == 'enable' ? 'has-sidebar' : 'no-sidebar'); ?> <?php echo esc_attr($archive_property_sidebar_position); ?> row">
	<?php if ($map_position == 'hide-map' || $map_position == 'map-header'): ?>
		<div
			class="<?php echo esc_attr($archive_property_sidebar == 'enable' ? $col_property : 'col-md-12') ?> properties-list-wrap">
					...
					<div class="col-lg-6 toolbar-search-list">
						...
						<div class="form-group">
							<a class="btn btn-display-properties-list"
								data-style-layout="<?php echo esc_attr($item_style_layout_list); ?>"
								data-style-layout-column="<?php echo esc_attr($column_layout_list); ?>"><i
		...
		<?php if ($archive_property_sidebar == 'enable'): ?>
			<div class="<?php echo esc_attr($col_sidebar); ?>">
	...
	<?php if ($map_position == 'half-map-right' || $map_position == 'half-map-left'): ?>
		...
		<div class="col-md-6">
			<div
				class="map-container <?php echo esc_attr(tfre_get_option('hide_map_mobile') == 'y' ? 'hide-map-mobile' : ''); ?>">
				<div id="map"></div>
			</div>
		</div>
	<?php endif; ?>
