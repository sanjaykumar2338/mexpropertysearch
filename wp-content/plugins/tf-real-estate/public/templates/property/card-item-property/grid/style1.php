			...
			<?php if ( ! empty( tfre_get_option( 'enable_card_agent' ) ) || ! empty( tfre_get_option( 'enable_card_year' ) ) ) : ?>
				...
				<div class="bottom">
					...
					<?php if ( ! empty( tfre_get_option( 'enable_card_year' ) ) && ! empty( $prop_year ) ) : ?>
						<span
							class="year"><?php echo wp_kses_post( empty( tfre_get_option( 'enable_convert_year' ) ) ? esc_html__( 'Built in: ', 'tf-real-estate' ) . $prop_year : tfre_get_year_time( $prop_year ) ); ?></span>
					<?php endif; ?>
				</div>
			<?php endif; ?>
		</div>
