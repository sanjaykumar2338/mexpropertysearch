<div class="item">
	...
	$width_image                  = $height_image = '495';
	$property_thumb               = tfre_image_resize_id( get_post_thumbnail_id( $property_id ), $width_image, $height_image, true );
	$list_gallery_images          = get_sources_property_gallery_images( $prop_gallery_images, false, 'style3' );
	...
	<div class="properties-post property-inner">
		<div class="featured-property card-image">
			<a class="view-gallery property-thumb" data-gallery="<?php echo esc_attr( json_encode( $list_gallery_images ) ); ?>" href="<?php echo esc_url( get_the_permalink() ); ?>">
				<?php
				$get_id_post_thumbnail = get_post_thumbnail_id( $property_id );
				if ( $toggle_lazy_load == 'on' ) {
					echo sprintf( '<img loading="lazy" class="lazy %2$s" src="" data-src="%1$s" data-image="%1$s" data-location="%3$s" data-id="%4$s" data-price="%5$s" data-price-prefix="%6$s" data-price-postfix="%7$s" title="%8$s" alt="image">', empty( $get_id_post_thumbnail ) ? TF_PLUGIN_URL . "includes/elementor-widget/assets/images/no-image-314-225.jpg" : tfre_image_resize_id( $get_id_post_thumbnail, $width_image, $height_image, true ), esc_attr( $class_image_map ), esc_attr( ! empty( $prop_address ) ? $prop_address : '' ), esc_attr( $property_id ), esc_attr( tfre_format_price( $prop_price_value, $prop_price_unit, true, $prop_enable_short_price_unit ) ), esc_attr( $prop_price_prefix ), esc_attr( $prop_price_postfix ), esc_attr( get_the_title() ) );
				} else {
					echo sprintf( '<img loading="lazy" class="%2$s" src="%1$s" data-image="%1$s" data-location="%3$s" data-id="%4$s" data-price="%5$s" data-price-prefix="%6$s" data-price-postfix="%7$s" title="%8$s" alt="image">', empty( $get_id_post_thumbnail ) ? TF_PLUGIN_URL . "includes/elementor-widget/assets/images/no-image-314-225.jpg" : tfre_image_resize_id( $get_id_post_thumbnail, $width_image, $height_image, true ), esc_attr( $class_image_map ), esc_attr( ! empty( $prop_address ) ? $prop_address : '' ), esc_attr( $property_id ), esc_attr( tfre_format_price( $prop_price_value, $prop_price_unit, true, $prop_enable_short_price_unit ) ), esc_attr( $prop_price_prefix ), esc_attr( $prop_price_postfix ), esc_attr( get_the_title() ) );
				}
				?>
			</a>
				...
				<ul class="list-controller">
					...
					<li>
						<a class="property-quick-view zoomGallery hv-tool"
							...
							data-target="#property_quick_view_modal"
