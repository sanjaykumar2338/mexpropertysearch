...
$item_per_page_advanced_search = tfre_get_option( 'item_per_page_advanced_search', '10' );
$args                          = array(
...
<?php if ( tfre_get_option( 'map_position' ) != 'hide-map' && tfre_get_option( 'map_position' ) == 'map-header' ) : ?>
	<div class="map-container <?php echo esc_attr( tfre_get_option( 'hide_map_mobile' ) == 'y' ? 'hide-map-mobile' : '' ); ?>" style="height:600px">
...
<div class="cards-container <?php echo esc_attr( tfre_get_option( 'map_position' ) ); ?> <?php echo esc_attr( tfre_get_option( 'archive_property_sidebar' ) == 'enable' ? 'has-sidebar' : 'no-sidebar' ); ?> <?php echo esc_attr( tfre_get_option( 'archive_property_sidebar_position' ) ); ?> row">
	<?php if ( tfre_get_option( 'map_position' ) == 'hide-map' || tfre_get_option( 'map_position' ) == 'map-header' ) : ?>
		<div class="<?php echo esc_attr( tfre_get_option( 'archive_property_sidebar' ) == 'enable' ? 'col-xl-9 col-md-12' : 'col-md-12' ) ?> properties-list-wrap">
				...
				<div class="row">
					...
					<div class="col-lg-6 toolbar-search-list">
						<div class="form-group">
							<a class="btn btn-display-properties-grid" data-style-layout="<?php echo esc_attr( tfre_get_option( 'item_style_layout_grid' ) ); ?>" data-style-layout-column="<?php echo esc_attr( tfre_get_option( 'column_layout_grid' ) ); ?>"><i class="far fa-th"></i></a>
						</div>
						<div class="form-group">
							<a class="btn btn-display-properties-list" data-style-layout="<?php echo esc_attr( tfre_get_option( 'item_style_layout_list' ) ); ?>" data-style-layout-column="<?php echo esc_attr( tfre_get_option( 'column_layout_list' ) ); ?>"><i class="far fa-list-ul"></i></a>
						</div>
						<div class="form-group">
							<select name="sortby" id="property_order_by" class="form-control" title="<?php esc_attr_e( 'Sort By', 'tf-real-estate' ) ?>">
						...
						<?php
						$enable_save_search = get_option( 'enable_save_search', 'y' );
						if ( $enable_save_search == 'y' ) :
							$data_target = '#save_search_advanced_modal';
							?>
							<div class="form-group advanced-saved-searches">
								<button type="button" class="tf-btn btn-save-search" data-toggle="modal" data-target="<?php echo esc_attr( $data_target ); ?>">
									<?php esc_html_e( 'Save Search', 'tf-real-estate' ); ?></button>
							</div>
							<?php tfre_get_template_with_arguments( 'global/save-search-advanced-modal.php', array( 'parameters' => $parameters, 'search_query' => $args ) );
						endif; ?>
					</div>
				</div>
			</form>
		...
		<?php endif; ?>
	<?php endif; ?>
	<?php if ( tfre_get_option( 'map_position' ) == 'half-map-right' || tfre_get_option( 'map_position' ) == 'half-map-left' ) : ?>
			...
			<form method="get" action="<?php echo esc_url( get_page_link() ); ?>" class="tfre-my-property-search">
				...
				<div class="row">
					...
					<div class="col-lg-6 toolbar-search-list">
						...
						<?php
						$enable_save_search = get_option( 'enable_save_search', 'y' );
						if ( is_user_logged_in() && $enable_save_search == 'y' ) :
							$data_target = '#save_search_advanced_modal';
							?>
							<div class="form-group advanced-saved-searches">
								<button type="button" class="tf-btn btn-save-search" data-toggle="modal" data-target="<?php echo esc_attr( $data_target ); ?>">
