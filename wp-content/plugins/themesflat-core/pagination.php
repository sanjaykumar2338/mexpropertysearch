		...
		if($style == 'numeric'){
			...
			<nav class="navigation navigation-numeric-link" role="navigation">
				...
				if (  $query->max_num_pages > 1 ){
					...
					'current' => max( 1, $paged ),
					'total' => $query->max_num_pages,				
					'prev_text' => ( '<i class="icon-dreamhome-angle-left"></i>' ),
					'next_text' => ( '<i class="icon-dreamhome-angle-right"></i>' ),
					));
				}
				?>
				</div>
			...
			<nav class="navigation navigation-link" role="navigation">
				...
				if (  $query->max_num_pages > 1 ){
					...
					'current' => max( 1, $paged ),
					'total' => $query->max_num_pages,
					'prev_text' => esc_html__( 'Previous', 'themesflat-core' ),
					'next_text' => esc_html__( 'Next', 'themesflat-core' ),
					));
				}
				?>
				</div>
			...
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                ...
                <nav class="navigation loadmore" role="navigation">
                    <div class="pagination loop-pagination <?php echo esc_attr($align); ?>">            
                    <a href=" <?php echo esc_url( get_next_posts_page_link() ); ?> "><?php echo esc_html__('Load More', 'themesflat-core'); ?></a>
                    </div>
                </nav>
                <?php
            }else {
                ...
                <nav class="navigation loadmore" role="navigation">
                    <div class="pagination loop-pagination <?php echo esc_attr($align); ?>">
                    <?php echo next_posts_link( esc_html__( 'Load More', 'themesflat-core' ), $query->max_num_pages ); ?>
                    </div>
                </nav>
                <?php
            }
