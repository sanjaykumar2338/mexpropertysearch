...
function themesflat_shortcode_default_id(){
    return array(
                ...
                'group' => esc_html__( 'Design Options', 'dreamhome' ),
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        $styles = apply_filters( 'themesflat/header_styles', array(
            'header-v1' => esc_html__( 'Classic', 'dreamhome' ),
            'header-v2' => esc_html__( 'Header style 02', 'dreamhome' ),
            'header-v4' => esc_html__( 'Modern', 'dreamhome' ),
        ) );
    }
...
function themesflat_available_social_icons() {
    $icons = apply_filters( 'themesflat_available_icons', array(
        'twitter'        => array( 'iclass' => 'icon-dreamhome2-twitter', 'title' => 'Twitter','share_link' => THEMESFLAT_PROTOCOL . '://twitter.com/intent/tweet?url=' ),
        'tik-tok'        => array( 'iclass' => 'icon-dreamhome2-tik-tok', 'title' => 'Tiktok','share_link' => THEMESFLAT_PROTOCOL . '://twitter.com/intent/tweet?url=' ),
        'facebook'       => array( 'iclass' => 'icon-dreamhome2-facebook', 'title' => 'Facebook','share_link'=> THEMESFLAT_PROTOCOL . '://www.facebook.com/sharer/sharer.php?u=' ),
        'google-plus'    => array( 'iclass' => 'icon-dreamhome2-google-plus', 'title' => 'Google Plus','share_link'=> THEMESFLAT_PROTOCOL . '://plus.google.com/share?url=' ),
        'pinterest'      => array( 'iclass' => 'icon-dreamhome2-pinterest', 'title' => 'Pinterest','share_link' => THEMESFLAT_PROTOCOL . '://pinterest.com/pin/create/bookmarklet/?url=' ),
        'instagram'      => array( 'iclass' => 'icon-dreamhome2-instagram', 'title' => 'Instagram','share_link' => 'https://www.instagram.com/?url=' ),
        'youtube'        => array( 'iclass' => 'icon-dreamhome2-youtube', 'title' => 'Youtube','share_link' =>'' ),
        'vimeo'          => array( 'iclass' => 'icon-dreamhome2-vimeo', 'title' => 'Vimeo','share_link' =>'' ),
        'behance'        => array( 'iclass' => 'icon-dreamhome2-behance', 'title' => 'Behance','share_link' =>'' ),
        'bitcoin'        => array( 'iclass' => 'icon-dreamhome2-bitcoin', 'title' => 'Bitcoin','share_link' =>'' ),
        'digg'           => array( 'iclass' => 'icon-dreamhome2-digg', 'title' => 'Digg','share_link' =>'http://digg.com/submit?url=' ),
        'skype'          => array( 'iclass' => 'icon-dreamhome2-skype', 'title' => 'Skype','share_link' => THEMESFLAT_PROTOCOL . '://web.skype.com/share?url='),
        'spotify'        => array( 'iclass' => 'icon-dreamhome2-spotify', 'title' => 'Spotify','share_link' => ''),
        'stack-overflow' => array( 'iclass' => 'icon-dreamhome2-stack-overflow', 'title' => 'Stack Overflow','share_link' => ''),
        'steam'          => array( 'iclass' => 'icon-dreamhome2-steam', 'title' => 'Steam','share_link' => ''),
        'dribble'          => array( 'iclass' => 'icon-dreamhome2-dribble', 'title' => 'Dribble','share_link' => ''),
        'linkedin'          => array( 'iclass' => 'icon-dreamhome2-linkedin', 'title' => 'Linkedin','share_link' => ''),
    ) );
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'dreamhome' ) . '</a></li></ul>';
}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'dreamhome' ), max( $paged, $page ) );
        }
...
function themesflat_custom_search_form( $form ) {
    ...
    <label>
        <span class="screen-reader-text">' . esc_html__( 'Search for:' , 'dreamhome' ) . '</span>
        <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Search…", "dreamhome" ) . '"/>
    </label>
...
function themesflat_get_page_titles() {
    ...
    if ( ! is_archive() ) {       
        if ( is_home() ) {
            ...
            if ( is_front_page() ) {
                $title = esc_html__('Blog', 'dreamhome');              
            }
        } 
        elseif ( is_page() ) {
            ...
            $title = esc_html__( '404', 'dreamhome' );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'dreamhome' ), get_search_query() );
        } else {
            $title = get_post_meta( get_the_ID(), 'custom_title', true );
            if ( ! $title ) {
                $title = get_the_title();
        ...
        if ( is_author() ) {
            the_post();
            $title = sprintf( esc_html__( 'Author: %s', 'dreamhome' ), get_the_author() );
            rewind_posts();
        } elseif ( is_day() ) {
            $title = sprintf( esc_html__( 'Daily: %s', 'dreamhome' ), get_the_date() );
        } elseif ( is_month() ) {
            $title = sprintf( esc_html__( 'Monthly: %s', 'dreamhome' ), get_the_date( 'F Y' ) );
        } elseif ( is_year() ) {
            $title = sprintf( esc_html__( 'Yearly: %s', 'dreamhome' ), get_the_date( 'Y' ) );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'dreamhome' ), get_search_query() );
        }
         elseif ( is_post_type_archive('emsb_service') ) {
            $title = esc_html__('Book Appointment','dreamhome');           
        }
